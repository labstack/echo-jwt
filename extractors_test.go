...
func TestTokenExtractionError_Is(t *testing.T) {
	given := echo.ErrUnauthorized.SetInternal(&TokenExtractionError{Err: errCookieExtractorValueMissing})

	assert.True(t, errors.Is(given, ErrJWTMissing))
	assert.True(t, errors.Is(given, errCookieExtractorValueMissing))
}

func TestTokenExtractionError_Error(t *testing.T) {
	given := &TokenExtractionError{Err: errCookieExtractorValueMissing}
	assert.Equal(t, "missing value in cookies", given.Error())
}

func TestTokenExtractionError_Unwrap(t *testing.T) {
	given := &TokenExtractionError{Err: errCookieExtractorValueMissing}
	assert.Equal(t, errCookieExtractorValueMissing, given.Unwrap())
}
